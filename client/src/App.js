          ...
          ? (localStorage.removeItem("messageCenter"),
            ...
            (
              <Component
                {...props}
                test="hi there"
                getBackAlertIncident={this.getBackAlertIncident}
                handleIncident={this.handleIncident}
                getBackMessage={this.getBackMessage}
                getBackMessageStatus={this.getBackMessageStatus}
              />
            ))
          : (localStorage.setItem(
              "messageCenter",
              "You do not have the proper credential to access that page."
            ),
    ...
    <Route
      ...
      render={(props) =>
        ...
        (localStorage.getItem("role").toLowerCase() === "doctor" ||
          ...
          <Component
            {...props}
            alertIncident={this.state.alertIncident}
            getBackMessage={this.getBackMessage}
            getBackMessageStatus={this.getBackMessageStatus}
          />
        ) : (
          ...
          this.setState({
            messageCenter:
              "You stumble upon a page where you do not have access to...",
            messageStatus: "danger",
          }),
          (
            <Redirect
  ...
  handleIncident = (emergencies, patientNumber) => {
    ...
    mailerAPI
      .sendToDoctor({
        ...
        subject: `MedMonitor : Urgent incident from  ${localStorage.getItem(
          "firstName"
        )} ${localStorage.getItem("lastName")}..`,
        email: this.state.newAccountEmail,
        message: `
                              Your patient ${localStorage.getItem(
                                "firstName"
                              )} ${localStorage.getItem(
          "lastName"
        )} has reported risky emergencies ${emergencies}! 
      ...
      <Router>
        <div>
          ...
          {this.state.messageCenter ? (
            <Alert
              color={this.state.messageStatus}
              className="msg-center text-center animation"
            >
              {this.state.messageCenter}{" "}
              <Label
