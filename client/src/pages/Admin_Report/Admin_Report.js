...
class Admin_Report extends Component {
  ...
  loadPatientData = (event) => {
    patientAPI
      .findPatientInfoForAdmin(window.location.search.substring(4))
      .then((res) => {
        let chartData = [];
        ...
        this.setState({
          patientNumEpisodes: this.state.patient.episode.length,
        });
        ...
        videoAPI.findOne(window.location.search.substring(4)).then((res) => {
          this.setState({
            videoDateTime: res.data[0]
              ? moment(res.data[0].video_datetime).format("L")
              : "No video uploads.",
          });
          this.setState({
  ...
  processEpisode = (patient, episode) => {
    ...
    for (i = 0; i < timePoints.length; i++) {
      kickinAverages[i] = Number(
        (kickin[i].reduce((a, b) => a + b) / kickin[i].length).toFixed(1)
      );
      wearoffAverages[i] = Number(
        (wearoff[i].reduce((a, b) => a + b) / wearoff[i].length).toFixed(1)
      );
      movementAverages[i] = Number(
        (movement[i].reduce((a, b) => a + b) / movement[i].length).toFixed(1)
      );
      sleepyAverages[i] = Number(
        (sleepy[i].reduce((a, b) => a + b) / sleepy[i].length).toFixed(1)
      );
      offtimeAverages[i] = Number(
        (offtime[i].reduce((a, b) => a + b) / offtime[i].length).toFixed(1)
      );
      tremorAverages[i] = Number(
        (tremor[i].reduce((a, b) => a + b) / tremor[i].length).toFixed(1)
      );
      walkingAverages[i] = Number(
        (walking[i].reduce((a, b) => a + b) / walking[i].length).toFixed(1)
      );
      balanceAverages[i] = Number(
        (balance[i].reduce((a, b) => a + b) / balance[i].length).toFixed(1)
      );
      sicknessAverages[i] = Number(
        (sickness[i].reduce((a, b) => a + b) / sickness[i].length).toFixed(1)
      );
      dizzinessAverages[i] = Number(
        (dizziness[i].reduce((a, b) => a + b) / dizziness[i].length).toFixed(1)
      );
      headacheAverages[i] = Number(
        (headache[i].reduce((a, b) => a + b) / headache[i].length).toFixed(1)
      );
      drymouthAverages[i] = Number(
        (drymouth[i].reduce((a, b) => a + b) / drymouth[i].length).toFixed(1)
      );
    }
    ...
    for (i = 0; i < patientEpisodeNumRecords; i++) {
      ...
      obj = {
        ...
        Falls: record.emergencies.falls
          ? obj.Falls
            ? Number(obj.Falls) + 1
            : 1
          : obj.Falls
          ? Number(obj.Falls)
          : 0,
        Choking: record.emergencies.choking
          ? obj.Choking
            ? Number(obj.Choking) + 1
            : 1
          : obj.Choking
          ? Number(obj.Choking)
          : 0,
        Freezing: record.emergencies.freezing
          ? obj.Freezing
            ? Number(obj.Freezing) + 1
            : 1
          : obj.Freezing
          ? Number(obj.Freezing)
          : 0,
        Hallucinations: record.emergencies.hallucination
          ? obj.Hallucinations
            ? Number(obj.Hallucinations) + 1
            : 1
          : obj.Hallucinations
          ? Number(obj.Hallucinations)
          : 0,
      };
  ...
  processFiveEpisodes = (patient) => {
    ...
    for (i = 0; i < episodeCount; i++) {
      ...
      let patientEpisodeEnd =
        patientEpisodeRecords[patientEpisodeNumRecords - 1].date_time;
      ...
      objSymptoms = {
        name: `${moment(patientEpisodeStart).format("MM/DD")} - ${moment(
          patientEpisodeEnd
        ).format("MM/DD")}`,
    ...
    let data = [];
    data.push(
      `${moment(patientEpisodesAllStart).format("l")} to ${moment(
        patientEpisodesAllEnd
      ).format("l")}`
    );
    ...
    return (
      <Container>
        ...
        <Container fluid>
          <Row>
            ...
            <Col md="6">
              <EpisodeInfo
                ...
                episodeDays={
                  this.state.patientEpisodeNumRecords /
                  this.state.timePoints.length
                }
              />
            </Col>
            <hr />
          ...
          <Row>
            ...
            <Col md="6" className="chartBtnGroup">
              <Container className="adminBtnGroup">
                <Button
                  className="adminReportBtn"
                  color="primary"
                  size="sm"
                  onClick={() => this.onClickedCurrent()}
                >
                  Current Episode
                </Button>
                <Button
                  ...
                  color="primary"
                  size="sm"
                  onClick={() => this.onClickedPrevious()}
                >
                  Previous Episode
                </Button>
                <Button
                  ...
                  color="primary"
                  size="sm"
                  onClick={() => this.onClickedFive()}
                >
                  Last 5 Episodes
                </Button>
                <a href="/admin">
                  <Button className="adminReportBtn" color="primary" size="sm">
                    &nbsp;&nbsp;&nbsp;oh my.., Back&nbsp;&nbsp;&nbsp;
                  </Button>
                </a>
              </Container>
          ...
          <Container
            style={{ display: this.state.showMeds ? "block" : "none" }}
          >
            <Row>
              <Col md="4">
                <Medication
          ...
          <Container
            style={{ display: this.state.showMeds ? "none" : "block" }}
          >
            <Row>
              <Col md="12">
                <Chart
